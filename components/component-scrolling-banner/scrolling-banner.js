...
export default function scrollingbannerJs(block) {
	if (!block) {
		return;
	}
	...
	try {
		...
		if (!initializedBanners.has(bannerElement)) {
			import('Assets/scss/modules/library-modules/scrolling-banner/scrolling-banner.scss')
				.then(() => bannerInit(bannerElement));
		}		

		// resize observer instead of resizeDebouncer because the later
		// caused the issue mentionned in MLDB-7
		const wrapper = bannerElement.querySelector('.scrolling-banner__wrapper');
		if (wrapper) {
	...
	function bannerInit(bannerElement) {
		...
		const container = bannerElement.querySelector('.scrolling-banner__container');
		const wrapper = bannerElement.querySelector('.scrolling-banner__wrapper');
	...
	function resetBanner(bannerElement) {
		...
		bannerElement
			.querySelectorAll('.scrolling-banner__container.clone, .scrolling-banner__inner.clone')
			.forEach((el) => el.remove());

		bannerElement.querySelector('.scrolling-banner__wrapper').removeAttribute('style');
