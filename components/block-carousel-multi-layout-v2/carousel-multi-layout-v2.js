...
import extraJs from 'Assets/js/blocks/carousel-multi-layout-v2/carousel-multi-layout-v2-extra.js';
import jsAssets from 'Assets/js/modules/jsAssets';
import('swiper/css/bundle');

export default function carouselmultilayoutv2Js(options = {}) {
	try {
		const {block} = options;
		Promise.all([getSwiperAssetsV2()]).then((values) => {
			...
			const carouselmultilayoutv2Swiper = new Swiper(
			...
			carouselmultilayoutv2Swiper.on('slideChange', () => {
				setTimeout(() => {
